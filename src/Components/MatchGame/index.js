...
class MatchGame extends Component {
  ...
  onToggleThumbnail = id => {
    ...
    const activeThumbnailList = imagesList.filter(
      eachList => eachList.imageUrl === imgUrl,
    )
    const isTrue = id === activeThumbnailList[0].id
  ...
  render() {
    const {isGameOver, category, score, time, imgUrl} = this.state
    console.log(time)
    const thumbnailList = imagesList.filter(
      eachValue => eachValue.category === category,
    )
    const ContentWhileGamePlaying = (
        ...
        <ul className="tab-elements">
          {tabsList.map(eachValue => (
            <TabsList
              key={eachValue.tabId}
              tabItem={eachValue}
              onToggleTabItem={this.onToggleTabItem}
              isActive={eachValue.tabId === category}
            />
          ))}
        </ul>
        <ul className="thumbnail-images">
          {thumbnailList.map(eachObject => (
            <Thumbnail
              key={eachObject.id}
              imgList={eachObject}
              onToggleThumbnail={this.onToggleThumbnail}
            />
          ))}
        </ul>
      </div>
    ...
    const GameStatus = isGameOver ? (
      <ScoreCard score={score} onTogglePlayAgain={this.onTogglePlayAgain} />
    ) : (
      ContentWhileGamePlaying
    )
    ...
    return (
      <div className="bg-container">
        ...
        <div className="content-div">{GameStatus}</div>
      </div>
    )
  }
}
